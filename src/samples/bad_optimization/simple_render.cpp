...
void SimpleRender::SetupSimplePipeline()
{
  ...
  m_pBindings->BindEnd(&m_cullDSet, &m_cullDSetLayout);
...
void SimpleRender::CreateCullingBuffers()
{
  ...
  m_instanceMatrices = vk_utils::createBuffer(m_device, sizeof(LiteMath::float4x4) * m_pScnMgr->InstancesNum(), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
  ...
  m_visibleInd = vk_utils::createBuffer(m_device, sizeof(uint32_t) * m_pScnMgr->InstancesNum(), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
...
void SimpleRender::FillCullingBuffers() 
{
  ...
  temp.push_back(m_pScnMgr->GetInstanceBbox(0));
  ...
  for (uint32_t i = 0; i < m_pScnMgr->InstancesNum(); i++)
    temp2.push_back(m_pScnMgr->GetInstanceMatrix(i));
  ...
  m_pCopyHelper->UpdateBuffer(m_instanceMatrices, 0, temp2.data(), sizeof(LiteMath::float4x4) * temp2.size());
...
void SimpleRender::CreateCullingPipeline()
{
  /////////////////////////////
  ...
  lambdaCreatePipeline("../resources/shaders/culling.comp.spv", sizeof(pushConstCulling), m_cullDSetLayout, m_cullingComputePipelineLayout, m_cullingComputePipeline);
}
                                            ...
                                            VkImageView, VkPipeline a_pipeline)
{
...

  {
    ...
    pushConstCulling.amount = m_pScnMgr->InstancesNum();
    vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_cullingComputePipeline);
    vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_COMPUTE, m_cullingComputePipelineLayout, 0, 1, &m_cullDSet, 0, NULL);
    vkCmdPushConstants(a_cmdBuff, m_cullingComputePipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pushConstCulling), &pushConstCulling);
...
void SimpleRender::ProcessInput(const AppInput &input)
{
  ...
  if(input.keyPressed[GLFW_KEY_B])
  {
    ...
    std::system("cd ../resources/shaders && python compile_bad_optimization_shaders.py");
#else
...
void SimpleRender::LoadScene(const char* path, bool transpose_inst_matrices)
{
  ...
  for (int i = -GRID_SIZE; i <= GRID_SIZE; ++i) {
    for (int j = -GRID_SIZE; j <= GRID_SIZE; ++j) {
      float4x4 matrix;
      //LiteMath::float3 offset = LiteMath::float3(j, -2, i);
      //LiteMath::float4x4 mat  = LiteMath::translate4x4(offset) * LiteMath::scale4x4(vec3(1, 1, 1));
