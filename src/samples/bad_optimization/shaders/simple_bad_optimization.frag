...
void main()
{
    ...
    vec3 absfractoff = abs(fract(surf.wPos.xyz) * 2.0 - 1.0);
    float maxComp = max(absfractoff.x, max(absfractoff.y, absfractoff.z));                                
    time_dependent_color *= pow(maxComp, 30);

    // Gradient red(left) -> blue(right) based on depth
